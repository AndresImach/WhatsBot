...
// POST { convId, valoracion: "positiva" | "negativa" | null } → guarda (o quita, con null) el 👍/👎 de la conversación.
export default async function handler(req, res) {
  if (req.method !== "POST") return res.status(405).json({ error: "Method not allowed" });
  if (!estaAutenticado(req)) return res.status(401).json({ error: "No autorizado" });

  const { convId, valoracion } = req.body || {};
  ...
  try {
    await setValoracion(convId, valor);
